...
export default function ModalRegister() {
  ...
  const [disabled, setRegister] = useState(false)
  ...
  // ----------------------------------------------------------------
 const Registered = (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const registerBtnOnClick = () => {
    let fnameOk,lnameOk,planError,genderOk = true;
    if (fname === "") {
      fnameOk = false;
      setFnameError(true);
    }else{fnameOk = true}
    ...
    if (fnameOk && lnameOk && planError && genderOk) {
      alert(
